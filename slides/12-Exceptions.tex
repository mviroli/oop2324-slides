...
\title[OOP12: Eccezioni]{12 \\ Errori di esecuzione ed Exceptions}
...
\frs{10}{Errori nei programmi}{
  \bl{Errori a tempo di compilazione (compile-time)}{\iz{
    ...
    \item editor moderni spesso li rendono ``edit-time''
    \item non sono considerabili ``bug'', ma situazioni temporanee nello sviluppo
  }}
  \bl{Errori a tempo di esecuzione (run-time) ($\Leftarrow$ oggetto della lezione)}{\iz{
    ...
    \item alcuni linguaggi (come Java/C\#, non il C) forniscono costrutti di alto livello per agevolarne la gestione
    \item nella programmazione moderna li si cerca di associare a bug
  }}
}

\fr{Errori per causa interna: lanciati dalla JVM}{
  \titledcode{Errore numerico}{12/code/Div.java}
  \titledcode{Overflow memoria}{12/code/Rec.java}
  \titledcode{Riferimento null}{12/code/Null.java}
}

\fr{Violazioni del contratto d'uso di un oggetto: librerie Java}{
  \titledcode{Operazione non supportata}{12/code/Op.java}
  \titledcode{Elemento non disponibile}{12/code/NoElement.java}
  \titledcode{Formato illegale}{12/code/Format.java}
}

\fr{Violazioni del contratto d'uso di un oggetto: nostro codice}{
  \titledcode{Argomento errato}{12/code/Arg.java}
  \titledcode{Elemento non disponibile}{12/code/It.java}
}
...
\fr{L'importanza della ``error-aware programming''}{
  \bl{Contratti}{\iz{
    ...
    \item Al di fuori queste condizioni è necessario interrompere il lavoro ed effettuare azioni correttive
  }}
...
\fr{Tipologie di errori in Java: UML}{
  \fg{height=0.85\textheight}{12/img/uml-throw.pdf}
}
...
\section{Realizzazione corretta di \cil{RangeIterator}}
...
\fr{Documentazione di \Cil{Iterator}: header}{
  \sizedcode{\ssmall}{12/code/util/Iterator1.java}
}

\fr{Documentazione di \Cil{Iterator}: \Cil{next()} e \Cil{hasNext()}}{
  \sizedcode{\ssmall}{12/code/util/Iterator2.java}
}

\fr{Documentazione di \Cil{Iterator}: \cil{remove()}}{
  \sizedcode{\ssmall}{12/code/util/Iterator3.java}
}

\fr{Documentazione generata: pt1}{
  \fg{height=0.9\textheight}{12/img/javadoc1.png}
}

\fr{Documentazione generata: pt2}{
  \fg{height=0.9\textheight}{12/img/javadoc2.png}
}
...
\section{Intercettare eccezioni}
...
\fr{\Cil{catch} multipli e \Cil{finally}}{
  \sizedcode{\scriptsize}{12/code/classes/UseRange3.java}
}
...
\section{Creazione e rilancio eccezioni}
...
\section{Dichiarazione eccezioni checked}
...
\fr{Checked vs Unchecked}{
  ...
  \bl{Checked: \cil{Exception} o sottoclassi ma non di \cil{RuntimeException}}{\iz{
    ...
    \item I metodi che le lanciano lo \alert{devono} dichiarare esplicitamente (\cil{throws})
...
\section{Applicazione domotica con eccezioni}
...
\fr{UML: Modellazione \Cil{Device}}{
  \fg{height=0.8\textheight}{12/img/uml-device.pdf}
}
...
\fr{UML: Design generale}{
  \fg{height=0.8\textheight}{12/img/dia.jpg}
}
