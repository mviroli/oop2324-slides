...
\frs{5}{Elementi delle lambda expression}{
  \bl{Che cos'è una lambda}{\iz{
    ...
    \item è applicabile a certi input (0, 1 o più), e dà un risultato (oppure \cil{void})
    ...
    \item ossia è una tecnica per ``passare del codice'' (come lo Strategy)
  }}
...
\fr{Esempio \Cil{SimpleIterator}}{
    \sizedrangedcode{\scriptsize}{5}{100}{\ecl/interfaces/SimpleIterator2.java}
}
...
\fr{Interfacce funzionali di libreria -- package \Cil{java.util.function}}{
    ...
    \bl{In \cil{java.util.function} vengono fornite varie interfacce ``general purpose''}{\iz{
      \item Sono tutte funzionali, e altamente generiche
...
\item \cil{java.lang.Runnable}: \texttt{run:()->void}, usato nella concorrenza
    }}    
...
\frs{10}{La classe \Cil{Optional}}{
    \bl{Il problema del \cil{NullPointerException}}{\iz{
      ...
      \item nella programmazione moderna, gestire l'assenza di una informazione con \cil{null} è considerato inappropriato: il \cil{null} non andrebbe proprio più usato.
    }}
    \bl{Idea}{\iz{
      ...
      \item \cil{Optional<T>} NON è serializzabile: serve quindi serailizzazione ad-hoc!
    }}
...
\frs{5}{Records}{
    \bl{Classi implementate come record}{\iz{
        \item un meccanismo introdotto in Java 14
        \item serve a dichiarare una classe con certi campi immutabili, e evitare tutto il boilerplate code di solito necessario
        \item promuove l'immutabilità dei dati, via sintassi concisa: da usare quando possibile!
    }}
    \bl{Meccanismo: \cil{record C(T1 x1, ..., Tn xn) implements I1,...,Ik\{...\}}}{\iz{
        \item realizzata come ``\cil{class C implements I1,...,Ik \{...\}}'', e\dots
        \item con campi privati e finali \cil{T1 x1}, ..., \cil{Tn xn}
        \item con costruttore con parametri \cil{x1...xn} -- costruttore ridefinibile
        \item con getters \cil{public T1 x1()} ... \cil{public Tn xn()}
        \item con \cil{equals}, \cil{hashcode} e \cil{toString}: usano tutti i campi
        \item non ammette ereditarietà, non ammette aggiunte di campi istanza
        \item si possono aggiungere altre cose: metodi, membri statici, eccetera
    }}
}

\fr{\Cil{record} example}{
  \sizedrangedcode{\tiny}{3}{100}{\ecl/records/TryRecords.java}
}
