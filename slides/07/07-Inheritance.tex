...
\newcommand{\ecl}{\idepath{p07inheritance}}
...
\fr{Alcuni scenari di riuso per le classi}{
  \bl{Riuso con estensione e/o sostituibilità}{\iz{
    \item Data una classe, realizzarne un'altra con caratteristiche solo in parte diverse (o nuove){\iz{
    \item avendo o non avendo a disposizione i sorgenti della classe originaria
    \item per crearne una specializzazione (ad esempio, una versione più robusta, o più prestante, o con più funzionalità)
    }}
    \item \dots ovviamente, senza violare il principio DRY
    \item La nuova classe potrebbe dover essere sostituibile alla precedente{\iz{
        \item fino a formare vere e proprie gerarchie ``is-a''
        \item ad esempio, data la classe \cil{Vehicle}, potrei poi creare le classi \cil{Car}, \cil{Truck}, \cil{Boat}, che sono tra loro abbastanza simili
    }}
  }}
}
...
\section{Riuso via composizione}


\fr{Esempio base: \Cil{Counter}}{
  %\sizedcode{\normalsize}{code/pre/Counter.java}
...
\fr{Una nuova classe senza riuso: \Cil{MultiCounter}}{
...
\fr{Riuso via composizione}{
  \bl{Uso di composizione/delegazione}{\iz{
    \item si incapsula un oggetto della classe da modificare/estendere
    \item si forniscono i metodi necessari, che posson essere più o meno
    \item alcuni di questi delegano il lavoro all'oggetto incapsulato
  }}
  \bl{Analisi del meccanismo}{\iz{
    \item nella programmazione moderna è considerata una \alert{ottima} soluzione
    \item possibile limite: esplicitare le delegazione vìola un poco DRY
    \item si può/deve usare in combinazione con le interfacce per gestire sostituibilità
  }}
  \bl{Possibile design per ottenere anche sostituibilità}{\iz{
    \item interfaccia \cil{CounterAbs}, implementazione \cil{CounterImpl}, e poi \cil{MultiCounterImpl} che implementa \cil{CounterAbs} e si compone di un \cil{CounterImpl} (nomi non ottimali)
  }}
}

\fr{Interfaccia \Cil{Counter} e classe \Cil{UseMultiCounter}}{
  \srcode{\scriptsize}{3}{32}{\ecl/composition/CounterAbs.java}
  \srcode{\scriptsize}{3}{32}{\ecl/composition/UseMultiCounter.java}
}

\fr{Classe \Cil{CounterImpl}}{
  \srcode{\footnotesize}{3}{32}{\ecl/composition/CounterImpl.java}
}

\fr{Classe \Cil{MultiCounterImpl}}{
  \srcode{\scriptsize}{3}{32}{\ecl/composition/MultiCounterImpl.java}
}
...
\frs{5}{Ereditarietà}{
  ...
  \bl{L'ereditarietà è un concetto chiave dell'OO}{\iz{
    \item È connesso al meccanismo delle interfacce
    \item È uno degli elementi chiave insieme a incapsulamento e interfacce
    \item Non riguarda solo il riuso di codice, ma influenza anche il polimorfismo conseguente, che è simile a quello delle interfacce e che descriveremo in dettaglio
  }}
...
\frs{15}{La necessità di estendere e modificare codice}{
  \bl{Una tipica situazione}{\iz{
    ...
    \item Appoggiarsi al ``copia e incolla'' di codice è \alert{sempre} sconsigliabile (principio DRY), perché tende a spargere errori in tutto il codice, e complica la manutenzione
    \item Ottenere riuso via composizione (ossia delegazione) è in generale una \alert{ottima soluzione}... ma ve ne sono di alternative, che possono avere dei vantaggi
  }}
  \bl{Si usa il meccanismo di ereditarietà}{\iz{
    ...
    \item Nota: non serve disporre dei sorgenti di \cil{D}, basta il codice binario, infatti in futuro estenderemo classi di libreria senza averne il sorgente
  }}
}
...
\fr{Notazione UML per l'estensione}{
  \bx{\iz{
    \item Arco a linea continua (punta a triangolo vuoto) per la relazione ``\cil{extends}''
...
\frs{5}{Livello d'accesso \Cil{protected}}{
  \bl{Usabile per le proprietà d'una classe}{\iz{
    \item \`E un livello intermedio fra \cil{public} e \cil{private}
    \item Indica che la proprietà (campo, metodo, costruttore) è accessibile dalla classe corrente, da una sottoclasse, e dalle sottoclassi delle sottoclassi (ricorsivamente)
    \item Cavillo: \cil{protected} in Java consente accesso anche da tutto il package, ovvero è ``meno stringente'' rispetto al livello di default package protected
  }}
  \bl{A cosa serve?}{\iz{
    ...
    \item Preferibile avere campi privati e getter/setter protetti
  }}
...
\fr{Campi \Cil{protected}? Meglio di no...}{
    \bl{Incapsulamento e \cil{protected}}{\iz{
        \item L'incapsulamente prevede di nascondere completamente l'implementazione (e quindi i campi) alle altre classi
        \item Un campo \cil{protected} violerebbe l'incapsulamento nei confronti della sottoclasse
        \item La modifica dell'implementazione in una sopraclasse rischia di propagarsi sulle sottoclassi
    }}
    \bl{Best practice}{\iz{
        \item Campi sempre privati
        \item Se l'accesso in lettura o scrittura è necessario solo nelle sottoclassi, allora utilizzare getter o setter \cil{protected}
    }}
}

\frs{2}{Estendibilità via getter/setter \Cil{proteceted}}{
  \srcode{\tiny}{6}{32}{\ecl/extendible/variant/ExtendibleCounter.java}
  \srcode{\tiny}{3}{32}{\ecl/extendible/variant/MultiCounter.java}
}
