...
\fr{Convenzioni sulla formattazione, pt 2}{
  \bl{Costrutti vari}{\iz{
    ...
    \item Chiusura graffa: in linea a sé, dove inizia la linea che la apre
    \item Metodi/costruttori separati da una linea vuota \\ (pochissime altre separazioni altrimenti)
    \item Usare graffe anche con blocchi ad uno statement
    \item Non usare assegnamenti dentro a espressioni
    \item Disambiguare priorità con parentesi solo in casi non banali
  }}
 \bl{Nomi}{\iz{
    \item Classi (e interface): iniziano con maiuscola
    \item Metodi, campi, variabili: iniziano con minuscola
    \item Se nome strutturato usare ``camelCase''/``PascalCase''
...
\fr{Convenzioni sulla formattazione, pt 3}{
  ...
  \bl{Nota finale}{\iz{
    ...
    \item Nelle slide è mostrato in questo modo salvo per necessità di spazio (generalmente segnalate)
...
\fr{Decomposizione, modularità e dipendenze}{
  \bl{Un punto cruciale della decomposizione è la ``modularità''}{\iz{
    ...
    \item nello specifico:{\en{
        \item ridurre le dipendenze
        \item favorire dipendenze intra-componenti rispetto a quelle inter-componente
        \item favorire dipendenze in componenti più innestati
    }}
  }}
...
\fr{Uso contatore, pt 2}{
  \bl{Altra possibilità}{\iz{
    ...
    \item In generale, si sono aperte nuove possibilità di riuso
  }}
