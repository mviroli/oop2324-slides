...
\title[OOP18: Stream funzionali]{18 \\ Stream funzionali}

\begin{document}

\newcommand{\ecl}{\idepath{p18streams}}
...
\frs{5}{Il concetto di Stream}{
  \bl{Idee}{\iz{
    ...
    \item Assomiglia molto al concetto di Iteratore, ma lo Stream è dichiarativo, perché non indica passo-passo come l'informazione viene processata, e quindi è concettualmente più astratto
    ...
    \item È possibile creare ``catene'' di trasformazioni di Stream (implementate con decorazioni successive) in modo funzionale, per ottenere flessibilmente computazioni non banali dei loro elementi, con codice più compatto e leggibile
    \item Questa modalità di lavoro rende molte computazioni (automaticamente) parallelizzabili, ossia computabili da un set arbitrario di Thread
  }}
}
...
\frs{10}{Computazioni con gli Stream}{
  \bl{Struttura a pipeline}{\iz{
    ...
    \item Una sequenza di trasformazioni:{\iz{
      \item mappe e filtri, ma non solo\dots
    }}
  ...
  \bl{Esempio: con persone con nome, città e reddito}{\iz{
    \item Data una \cil{List<Person>} con proprietà reddito e città, ottenere la somma dei redditi di tutte le persone di Cesena
...
\fr{Classe \Cil{Person} -- \Cil{equals}, \Cil{hashCode} e \Cil{toString} omessi}{
    \sizedrangedcode{\tiny}{5}{34}{\ecl/Person.java}
}


\fr{Realizzazione dell'esempio in Java 8}{
    \sizedrangedcode{\tiny}{5}{100}{\ecl/UseStreamsOnPerson.java}
}
...
\fr{La libreria degli Stream}{
  \bl{Struttura}{\iz{
    ...
    \item Interfaccia \cil{Collector<T,A,R>}: rappresenta una operazione di rid uzione
    \item Classe \cil{Collectors}: fornisce una serie di collettori
    \item \dots altre classi di Java creano degli stream
  }}
}

\fr{Le collection generano Stream!}{
    \sizedrangedcode{\scriptsize}{1}{100}{../17/snippets/Collection.java}
}

\fr{L'interfaccia \Cil{java.util.BaseStream}}{
    \sizedrangedcode{\ssmall}{1}{100}{../17/snippets/BaseStream.java}
}
...
\frs{5}{Riassunto delle funzionalità di una pipeline per \Cil{Stream<X>}}{
    ...
    \bl{Trasformazione}{\iz{
      \item \cil{filter}, \cil{map}, \cil{flatMap}, \cil{distinct}, \cil{sorted}, \cil{peek}, \cil{limit}, \cil{skip}, \cil{mapToXYZ},\dots
    }}
    \bl{Terminazione}{\iz{
      \item \cil{forEach}, \cil{forEachOrdered}, \cil{toArray}, \cil{reduce}, \cil{collect}, \cil{min}, \cil{max}, \cil{count}, \cil{anyMatch}, \cil{allMatch}, \cil{noneMatch}, \cil{findFirst}, \cil{findAny},\dots
    }}
...
\fr{\Cil{java.util.Stream}: costruzione stream, 1/3}{
    \sizedrangedcode{\tiny}{1}{26}{../17/snippets/Stream.java}
}

\fr{\Cil{java.util.Stream}: trasformazione stream, 2/3}{
    \sizedrangedcode{\tiny}{26}{45}{../17/snippets/Stream.java}
}

\fr{\Cil{java.util.Stream}: terminazione stream, 3/3}{
    \sizedrangedcode{\tiny}{46}{100}{../17/snippets/Stream.java}
}
...
\fr{Terminazione generalizzata con \Cil{Stream.collect}}{
    \sizedrangedcode{\tiny}{6}{100}{\ecl/UseGeneralizedCollectors.java}
}

\fr{Collettori di libreria: la classe \Cil{Collectors}}{
    \sizedrangedcode{\tiny}{1}{100}{../17/snippets/Collectors.java}
}
...
\fr{Esempi avanzati su \Cil{Person}}{
    \sizedrangedcode{\tiny}{6}{100}{\ecl/UseStreamsOnPerson2.java}
}

  \fr{Algoritmi funzionali -- cosa realizzano?}{
    \sizedrangedcode{\tiny}{7}{100}{\ecl/UseStreamsForAlgorithms.java}
}
...
\fr{Stream e parallelismo}{
  \bl{Uno dei vantaggi degli stream}{\iz{
    \item È possibile gestire in modo completamente automatico il parallelismo, e quindi la disponibilità di più core
    \item Alcuni stream (ossia non tutti) ammettono implementazioni multi-threaded efficienti{\iz{
      \item \cil{range}, collezioni\dots ma non \cil{iterate}
    }}
    \item Gli effettivi vantaggi (o addirittura penalizzazioni!) dipendono da molti fattori{\iz{
      ...
      \item Sempre meglio verificare se c'è vantaggio prima di usarli
    }}
  }}
  \bl{Come si abilita il parallelismo (ove possibile)?}{\iz{
    \item Basta richiamare il metodo \cil{parallel()} su uno stream\dots
    \item \dots o il metodo \cil{parallelStream()} da una collection
  }}
}

\fr{Test concurrency}{
    \sizedrangedcode{\ssmall}{8}{100}{\ecl/TestConcurrency.java}
}

\fr{Considerazioni sul guadagno di performance -- dati molto variabili\dots}{
  \bl{Nel caso specifico (PC 6 cores), esecuzione ripetuta $1'000$ volte}{\iz{
    \item Con $10'000$ elementi nella collection, rapporto circa $0.9$ (più lento)
    \item Con $100'000$ elementi nella collection, rapporto circa $3.7$ 
    \item Con $1'000'000$ elementi nella collection, circa $6.5$
  }}
  \bl{Indicazioni generali}{\iz{
    \item Usare parallelismo solo con istanze di dimensione significativa
    \item È bene se le lambda usate nelle trasformazioni sono semplici
...
\fr{Interfaccia Spliterator}{
  \sizedrangedcode{\tiny}{1}{100}{../17/snippets/Spliterator.java}
}

\fr{Esempio: \Cil{Streams.RangeSpliterator}}{
  \sizedrangedcode{\tiny}{1}{100}{../17/snippets/RangeSpliterator.java}
}
