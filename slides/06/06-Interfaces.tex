...
\title[OOP06: Interfacce]{06 \\ Composizione, riuso e sostituibilità: le \texttt{interface}}

\begin{document}

\newcommand{\ecl}{\idepath{p06interfaces}}
...
\fr{Outline}{
  ...
  \item Notazione UML
  \item \cil{interface} in Java e meccanismi collegati
...
\fr{Composizione -- relazione ``has-a''}{
  \bl{Idea}{\iz{
    \item un oggetto della classe \cil{A} è ottenuto componendo un insieme di altri oggetti, delle classi \cil{B1}, \cil{B2}, \dots, \cil{Bn} 
    \item si dice che un oggetto di \cil{A} contiene, o si compone di, oggetti delle classi \cil{B1}, \cil{B2}, \dots, \cil{Bn}
    \item ossia, lo stato dell'oggetto di \cil{A} include le informazioni relative allo stato di un oggetto di \cil{B1}, uno di  \cil{B2}, \dots, uno di \cil{Bn}
    \item si noti che si parla propriamente di composizione quando \cil{B1}, \cil{B2}, \dots, \cil{Bn} non sono tipi primitivi, ma classi
...
\fr{Qualche esempio di composizione}{
  ...
  \bl{Ateneo}{
    Un oggetto ateneo si compone di oggetti di tipo \cil{Faculty}, \cil{Student}, \cil{Teacher}, eccetera
  }
...
\fr{Un esercizio: dispositivo \Cil{TwoLampsDevice}}{
  \bl{Caratteristiche del sistema da modellare}{\iz{
    ...
    \item possibilità di modalità ``eco'' (una sola accesa, a metà)
  }}
...
\fr{Class Diagram}{
  \bl{\dots diagramma delle classi, una prima descrizione}{\iz{
...
\frs{5}{Motivazioni}{
  \bl{Specifica}{\iz{
    ...
    \item Consente di separare bene ``abstraction'' e ``concretion''
  }}
  \bl{Polimorfismo}{\iz{
    \item Serve un meccanismo per poter fornire diverse possibili realizzazioni di un contratto (o sue versioni implementative successive)
...
\frs{10}{Razionale delle interfacce}{
  \bl{Quando costruire una interfaccia?}{\iz{
    ...
    \item quando si prevede che una classe possa essere evoluta in una nuova versione, come ulteriore implementazione
...
\fr{Sottotipi e principio di sostituibilità}{
  \bl{Principio di sostituibilità di Liskov (1993)}{
    Se \texttt{A} è un sottotipo di \texttt{B} allora ogni oggetto (o valore) di \texttt{A} deve essere utilizzabile dove un programma si attende un oggetto (o valore) di \texttt{B}
  }
  ...
  \bl{Si rischiano errori?}{
    Non per via di chiamate a metodi non definiti! Il programma può manipolare gli elementi del tipo \texttt{I} solo mandando loro i messaggi dichiarati in \texttt{I}, che sono sicuramente ``accettati'' dagli oggetti di \texttt{C}. Il viceversa non è vero.
   }
