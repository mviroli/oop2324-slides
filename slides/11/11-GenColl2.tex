...
\newcommand{\ecl}{\idepath{p11gencoll2}}
...
\frs{5}{Limitazioni all'uso dei generici}{
...
\fr{Uso di \Cil{Vector<X>}}{
  \sizedrangedcode{\scriptsize}{3}{100}{\idepath{p09generics}/generics/UseVector.java}
}

\fr{Implementazione di \Cil{Vector} pt 1}{
\sizedrangedcode{\ssmall}{3}{26}{\idepath{p09generics}/generics/Vector.java}
}

\fr{Implementazione di \Cil{Vector} pt 2}{
\sizedrangedcode{\ssmall}{27}{100}{\idepath{p09generics}/generics/Vector.java}
}

\frs{5}{Ancora sugli ``unchecked warning'' coi generici}{
\sizedrangedcode{\scriptsize}{3}{100}{\ecl/abstractions/ShowCast.java}
\bl{Unchecked warning e ``unsafety''}{\iz{
    \item un unchecked warning per via di conversione con genericità espone l'esecuzione dei nostri programmi a possibili errori successivi (così come i downcast)
    \item usarli solo se si è certi ciò non succeda (in tal caso vedremo come ``silenziare il warning''
}}
}
...
\frs{5}{Subtyping e safety}{
  \bl{\alert{Safety} di un linguaggio OO}{
    Se nessuna combinazione di istruzioni porta a poter invocare un metodo su un oggetto la cui classe non lo definisce{\iz{
      ...
      \item In Java bisogna evitare gli usi di downcast e unchecked cast
    }}
    Più in generale, se non possono accadere errori a tempo di esecuzione\dots
  }
  \bl{Java}{\iz{
...
\fr{Unsafety con gli array di Java}{
  \bl{Fin da Java 1, gli array di Java sono stati trattati come covarianti!}{\iz{
    ...
    \item Quindi ogni scrittura su array potrebbe successivamente potenzialmente fallire\dots{} lanciando un \cil{ArrayStoreException}
  }}
  \sizedcode{\footnotesize}{../09/code/Arrays.java}
}

\frs{5}{Covarianza e operazioni di accesso}{
  ...
  \bl{I wildcard types:}{\iz{ 
    \item Sono delle ``interfacce'' sopratitpo dei tipi generici, per le quali invece vale co-/contro-varianza
  }}
}
...
\fr{Classi di utilità (moduli): \Cil{Arrays} e \Cil{Collections}}{
  ...
  \bl{\cil{java.util.Collections}}{\iz{
    ...
    \item Wrapper immutabili: unmodifiableList, unmodifiableSet
