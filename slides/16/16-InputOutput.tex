...
\title[OOP16: I/O]{16 \\ Input-Output}

\begin{document}

\newcommand{\ecl}{\idepath{p16io}}
...
\frs{5}{Il problema dell'Input/Output}{
  ...
  \bl{La libreria \cil{java.io.*}}{\iz{
    ...
    \item È estesa con alcune funzionalità di facciata nella libreria \cil{java.nio}
    \item È la base di librerie avanzate (networking,\dots), anche non-JDK (JSON,\dots)
...
\section{File e loro proprietà}

\frs{5}{I File}{
    \bl{File system}{\iz{
	\item Il file system è un modulo del S.O. che gestisce la memoria secondaria
	\item Maschera le diversità di dispositivi fisici (HD, CD, DVD, BR, SSD,\dots)
	\item Maschera le diversità di contenuti informativi (testi, filmati, archivi,\dots)
	\item Fornisce meccanismi per fornire prestazioni, concorrenza, robustezza
    }}
    \bl{File}{\iz{
	\item Un file system contiene/gestisce un insieme di \alert{file}
	\item Un file ha un contenuto informativo, tipicamente una sequenza di byte{\iz{
...
getName prova.bin
getParent /home/mirko/aula/oop
isAbsolute true
getCanonicalPath /home/mirko/aula/oop/prova.bin
getPath /home/mirko/aula/oop/prova.bin
getParentFile /home/mirko/aula/oop
getAbsolutePath /home/mirko/aula/oop/prova.bin
getAbsoluteFile /home/mirko/aula/oop/prova.bin
getCanonicalFile /home/mirko/aula/oop/prova.bin
canRead true
canWrite true
isDirectory false
isFile true
...
\fr{Overview sugli \Cil{InputStream} e \Cil{OutputStream} in Java}{
	...
	\item Sono classi astratte (e non interfacce\dots)
	\item Possono essere specializzate da ``sottoclassi'' e ``decorazioni'', tra cui{\iz{
	    ...
	    \item[$\Rightarrow$]\dots e corrispondenti versioni \cil{Output}
	}}
    }}
...
\fr{Il costrutto \Cil{try-with-resources}}{
    \bl{Costrutto \cil{try-with-resources}}{\iz{
      \item vuole la definizione/creazione di un \cil{java.lang.AutoCloseable} come primo argomento
      \item ne assicura la chiusura 
      \item si possono opzionalmente aggiungere delle \cil{catch} di eccezioni
      \item andrebbe sempre usato\dots
    }}
...
% 	\item Ove possibile, ciò dovrebbe essere possibile da qualunque stream
	...
	\item Un oggetto di tale nuova classe è un decoratore per quello interno\dots visto che ne modifica il funzionamento
...
\fr{Altri decoratori di \Cil{InputStream} (\dots e \Cil{OutputStream})}{
...
\fr{Ancora sui decoratori}{
	...
	\item Uno stream di sorgente dati: \cil{FileInputStream}, \cil{ByteArrayInputStream}, \dots
	\item Uno (o più) stream di gestione interna: \cil{BufferInputStream}, \dots
	\item Uno stream di presentazione dati: \cil{DataInputStream}, \cil{ObjectInputStream}, \dots
    }}
}

\section{Serializzazione di oggetti}
...
\frs{5}{Classe \Cil{Person} e l'interfaccia ``tag'' \Cil{Serializable}}{
    \sizedrangedcode{\ssmall}{3}{100}{\ecl/files/Person.java}
...
\fr{Il problema delle versioni di una classe: \Cil{serialVersionUID}}{
    \bl{Problema}{\iz{
	\item Si serializza un oggetto, la classe viene modificata e ricompilata, e quindi si ritira su l'oggetto\dots  i dati sarebbero facilmente ``corrupted''
    }}
...
\fr{\Cil{CPerson} con caching \Cil{toString}, pt1}{
    \sizedrangedcode{\ssmall}{3}{29}{\ecl/trans/CPerson.java}
}

\fr{\Cil{CPerson} con caching \Cil{toString}, pt2}{
    \sizedrangedcode{\ssmall}{31}{100}{\ecl/trans/CPerson.java}
}
...
\fr{\Cil{APerson}: serializzazione ad-hoc per una data}{
    \sizedrangedcode{\tiny}{6}{35}{\ecl/trans/APerson.java}
}
...
\fr{Limitazione degli stream binari visti}{
  ...
  \bl{Leggibilità e intelligibilità} {
    \iz {
      ...
      \item Esistono standard per scrivere dati in formato testuale
      \item Problema da risolvere in due fasi:
      \en {
        \item Conversione da dato a stringa di testo e viceversa
...
\fr{Conversione da oggetto a stringa di testo e viceversa} {
  ...
  \bl{(Alcuni) Formati standard per la conversione di oggetti in testo} {
    \iz {
      \item JavaScript Object Notation -- JSON (RFC 7149)
      \iz {
...
\fr{File ed encoding}{
    \bl{Il contenuto dei file} {
      Supponiamo di utilizzare un \cil{DataOutputStream} per scrivere in sequenza i numeri da 0 a 20 (escluso)
        \iz {
            \item Quali bit vengono scritti in realtà? Non è noto, dipende dall'implementazione della libreria\dots
            \item Ossia, si usa una codica non standard e non ``open''
        }
    }
    \iz {
