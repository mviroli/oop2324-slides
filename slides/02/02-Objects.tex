...
\fr{``(Almost) Everything is an object''}{
  ...
  \fg{width=0.6\textwidth}{img/primitive.png}
}
...
\fr{Oggetti e memoria}{
  \bl{Gestione della memoria -- entriamo nella JVM solo temporanemante}{\iz{
    ...
    \item le variabili sono allocate nello stack, nei rispettivi record di attivazione
...
\fr{Visibilità, ossia ``scope'', delle variabili}{
  \bl{Definita una variabile, dove sarà visibile?}{\iz{
    \item Regole molto simili a quelle del C
  ...
  \bl{Tempo di vita degli oggetti}{\iz{
    \item finita l'esecuzione delle istruzioni relative allo scope di una variabile, l'oggetto continua a esistere
...
\fr{Costruire classi}{
  \bl{Premesse}{\iz{
    ...
    \item progettare e costruire classi correttamente sarà l'obiettivo del corso
    \item incominciamo descrivendo la loro struttura generale
    \item daremo via via linee guida sempre più precise
  }}
...
\fr{Struttura di una classe}{
  ...
  \bl{Membri della classe}{
    \alert{Campi} (in inglese, fields){\iz{
      \item descrivono la struttura/stato dell'oggetto in memoria
    }}
    \alert{Metodi} (in inglese, methods){\iz{
...
\fr{Campi: un esempio ``toy'' (giocattolo)}{
  \titledcode{Classe}{code/fields1.java}
  \titledcode{Uso}{code/fields2.java}
}

\fr{Campi: un esempio realistico, \Cil{Point3D}}{
...
\fr{Metodi}{
  \bl{Elementi costitutivi dei metodi}{\iz{
    ...
    \item ognuno ha una \alert{intestazione} (signature) e un corpo (body){\iz{
...
\fr{La variabile speciale \Cil{this}}{
  \bl{\cil{this}}{\iz{
    ...
    \item per rendere meno ambigua la sintassi, Java fornisce una variabile speciale denotata con \cil{this}, che contiene il riferimento all'oggetto che sta gestendo il messaggio corrente
    \item per motivi di leggibilità, è opportuno usarla sempre in questo corso
  }}
...
\fr{Package e librerie Java: organizzazione}{
  \bl{Librerie di Java}{\iz{
    \item Documentazione auto-generata, consultabile offline, o online: \\ \myurl{https://docs.oracle.com/en/java/javase/17/docs/api/} \\ (google search ``\texttt{javadoc 17}'')
    \item contano 4000+ classi, raggruppate in 200+ \alert{package} (e 50+ \alert{moduli})
  }}
  \bl{Package}{\iz{
...
\frs{5}{Package, moduli e librerie Java: uso}{
  ...
  \bl{Classi (e funzionalità) ``deprecate''}{\iz{
    ...
    \item noi le utilizzeremo a volte per scopi didattici, solo a inizio corso
