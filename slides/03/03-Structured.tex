...
\title[OOP03: Prg. strutturata (C-like) in Java]{03 \\ Programmazione strutturata (C-like) in Java}

\begin{document}

\newcommand{\ecl}{\idepath{p03structured}}
...
\frs{10}{Tipi}{
  ...
  \item Classi: \cil{Object}, \cil{String}, \cil{ArrayList}, \cil{JFrame},$\ldots$
  \item Altri che vedremo in seguito: interfacce, classi innestate, generici,...
  }}
  \bl{Java e i tipi\dots}{\iz{
    \item diversamente da linguaggi dinamici (ad esempio, JavaScript, Python,$\ldots$)
    \item Java ha ``typing statico'': ogni espressione ha un tipo noto al compilatore
    \item e ``typing forte'': non si accettano espressioni con tipo diverso da quello atteso
    \item[$\Rightarrow$]\dots permette l'intercettazione a priori di molti errori
    \item[$\Rightarrow$]\dots disciplina progettazione e programmazione
  }}
}
...
\frs{5}{Interi: \Cil{byte}, \Cil{short}, \Cil{int}, \Cil{long}}{
  \bl{Operatori}{\iz{
  ...
  \bl{Codifica, rappresentazione (di fatto, come in C)}{\iz{
  ...
  \bl{Prassi}{\iz{
    \item Raro l'uso di \cil{short}, \cil{byte} usato per realizzare array di grosse dimensioni
...
\fr{Numeri in virgola mobile: \Cil{float}, \Cil{double}}{
  \bl{Operatori}{\iz{
  \item \cil{+}, \cil{-}, \cil{*}, \cil{/} (con resto), \cil{\%} (resto), \cil{+} e \cil{-} anche unari
  }}
  \bl{Codifica, rappresentazione}{\iz{
    \item Codificati secondo lo standard IEEE 754 (come in C)
...
\fr{Il tool \Cil{JShell}}{
  \bl{JShell (JDK 9+)}{\iz{
    ...
    \item è una console Java, dove si possono eseguire istruzioni vedendone subito l'effetto\dots via via
...
\fr{Java array}{
  ...
  \item È impossibile violare i limiti di una array, pena un errore a tempo di esecuzione (\cil{ArrayIndexOutOfBoundsException})
  \item L'accesso ad un array è di conseguenza leggermente rallentato, almeno in linea di principio, rispetto al caso del C
  }}
}

\frs{5}{Sintassi Array}{
  \bl{Creazione array}{\iz{
  \item Due notazioni: creazione per elenco, e per dimensione{\iz{
...
\frs{5}{Array di oggetti}{
  \bl{Creazione array di oggetti -- stessa notazione}{\iz{
  \item Per elenco:{\iz{
  ...
  \item frequente errore è pensare che sia un array di nuovi oggetti automaticamente creati
...
\fr{I linguaggi OO sono anche imperativi/strutturati}{
  ...
  \item Come C++ e C\#, Java è alla base (ossia quando si compone il corpo di un metodo) imperativo/strutturato -- altri linguaggi come Scala invece no
...
\fr{Java vs C}{
  ...
  \item Java è molto più restrittivo del C, ossia fa controlli più dettagliati
...
\fr{Java come linguaggio puramente strutturato}{
  \bl{Un uso limitato (ma a volte utile) di Java}{\iz{
    ...
    \item È una tecnica usata per realizzare ``utility class'', come ad esempio la classe delle funzioni matematiche \cil{java.lang.Math}
  }}
}
...
\fr{Un esercizio sugli array}{
  \bx{Costruire una funzione che dato un array ne produce in uscita uno della stessa lunghezza, ma invertendo il primo elemento con l'ultimo, il secondo col penultimo, etc\dots}
...
\fr{For-each}{
  \bl{Java introduce una variante del ciclo \cil{for}}{\iz{
   \item supporta l'astrazione di ``per ogni elemento della collezione fai\dots''
   \item utile con gli array quando non importa il valore corrente dell'indice
   \item utilizzabile anche con le Collection di Java (liste, insiemi,\dots)
  }}
  \bl{Sintassi -- caso di array di interi}{\iz{
   \item \cil{for(int v: array)\{ /* un body che usa v */ \}}
   \item spesso usato con \cil{var}: \cil{for(var v: array)\{ /* un body che usa v */ \}}
   \item \cil{array} è una espressione che restituisce un \cil{int[]}
   \item nel corpo del \cil{for}, \cil{v} è un \cil{int}: vale via via ogni elemento dell'array
...
\fr{Elementi applicativi all'interno del corso OOP}{
  \bl{Teoria o applicazioni?}{\iz{
    ...
    \item Si mostreranno qualche volta semplici applicazioni di esempio
  }}
}
...
\fr{Applicazione: \Cil{GuessMyNumberApp}}{
  ...
  \bl{Elementi implementativi}{\iz{
    \item \cil{System.console().readLine} usabile per leggere input da tastiera
...
\section{Un addendum: sulla qualità del software\dots}
...
\fr{Come è fatto del ``buon software''?}{
  ...
  \bl{Qualità esterna -- aspetti non-funzionali}{\iz{
    ...
    \item Caratteristiche di sicurezza, usabilità, etc\dots
  }}
...
\fr{Convenzioni}{
  \bl{Convenzioni sul codice}{\iz{
    ...
    \item \myurl{https://www.oracle.com/technetwork/java/codeconventions-150003.pdf}
    \item Alcuni team usano convenzioni ancora più ristrette (Google)
  }}
  \bl{Li vedremo via via\dots intanto focalizziamoci sulla formattazione}{\iz{
    \item Una istruzione per linea
    \item Formattazione parentesi graffe (È CRUCIALE!)
...
\fr{Una nota sulle performance}{
  \bl{Performance e JVM}{\iz{
    ...
    \item Di recente, tecniche avanzate nelle VM hanno ridotto, se non in alcuni casi annullato o migliorato, le differenze in performance
  }}
  \bl{In questo corso}{\iz{
    \item Non ci occuperemo in dettaglio di questo aspetto
    \item Prediligeremo sempre la soluzione più semplice/compatta/coerente
