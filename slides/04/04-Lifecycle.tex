...
\newcommand{\ecl}{\idepath{p04objects}}
...
\frs{5}{Codice statico}{
  ...
  \item Tali campi e metodi sono considerati proprietà della classe, o comunque dell'insieme di tutti gli oggetti, non di un singolo oggetto
...
\fr{Uso di codice statico o non statico in una classe?}{
  \bl{Codice non statico (detto anche codice istanza)}{\iz{
  \item È codice puro object-oriented
  \item Definisce le operazioni e lo stato di ogni oggetto creato dalla classe
  }}
  \bl{Codice statico}{\iz{
  \item Non è codice puro object-oriented, ma codice in stile ``imperativo/strutturato''
  \item Definisce funzioni e variabili del software definito dalla classe{\iz{
    \item possono essere visti come metodi e campi della classe (non dei suoi oggetti)
  }}
  }}
  \bl{Usarli assieme?}{\iz{
    \item Le helper class abbiano solo metodi/campi statici
    \item Le classi OO non abbiano metodi/campi statici, a meno di qualche funzionalità ``generale'' della classe (si veda nel prosieguo)
  }}
}
...
\fr{Point3D: commenti}{
    ...
    \bl{Razionale}{\iz{
      ...
      \item \cil{zero} è una sorta di ``costante'', quindi non è proprietà di un oggetto
    }}
...
\fr{Variante Point3DBis\dots inizializzazione con ritorno}{
...
\fr{Sull'uso delle proprietà \Cil{static}}{
    \bl{Consigli generali}{\iz{
      ...
      \item le librerie di Java ne fanno largo uso (troppo largo forse)
    }}
    \bl{Prassi frequente nelle librerie di Java}{\iz{
    ...
    \bl{In altri linguaggi\dots come Scala}{\iz{
...
\fr{Uso \Cil{Point3D} e \Cil{Points}}{
    \srcode{\ssmall}{3}{100}{\ecl/points/UsePoint3D.java}
}
...
\fr{Esempio: Person (3 costruttori)}{
  %\sizedcode{\scriptsize}{code/Persona.java}
  \srcode{\ssmall}{3}{100}{\ecl/Person.java}
}

\fr{Esempio: Uso di Person (3 costruttori)}{
  \srcode{\scriptsize}{3}{100}{\ecl/UsePerson.java}
...
\fr{Istruzione \Cil{this(...)}}{
  ...
  \srcode{\ssmall}{3}{100}{\ecl/Person2.java}
}

\fr{Overloading dei costruttori}{
  \bl{Overloading}{\iz{
    \item È la tecnica che consente di dare più significati ad uno stesso ``nome''
  ...
  \bl{Overloading dei costruttori}{
    ...
    \iz{
      ...
      \item se ve ne è uno allora è quello che verrà chiamato\dots
...
\fr{I package pt. 2}{
 \bl{Dichiarazione del package}{\iz{
   ...
   \item Lo fa all'inizio del sorgente con la direttiva \cil{package pname;}
...
\frs{5}{La keyword \Cil{final}}{
 ...
 \bl{Il caso più usato: costanti di una classe}{\iz{
    ...
    \item Nota la formattazione
      }}
 }}
 \bl{Il caso dei ``magic number''}{\iz{
    \item Ogni numero usato in una classe per qualche motivo (3,21,\dots)\dots
    \item \dots è sarebbe opportuno sia mascherato da una costante, per motivi di leggibilità e di più semplice modificabilità
 }}
}
