...
\fr{Casi di sistemi software}{
  \bl{Esempi}{\iz{
    ...
    \item Simulatore di movimento di folle, dinamica pianeti, \dots
    \item Monitor e elaborazione di dati biometrici (pressione, dati cardiaci, \dots)
...
\fr{Concetti del solution/problem space: ``livello di astrazione''}{
  \bl{Definizione di astrazione (nell'informatica)}{\iz{
     \item \`E il metodo usato per descrivere la parte importante di un sistema informatico complesso con lo scopo di facilitarne la progettazione, implementazione e manutenzione: si basa sul mettere da parte alcune caratteristiche specifiche ritenute non essenziali, concentrandosi su quelle invece cruciali.
...
\frs{5}{Il livello d'astrazione dei linguaggi di programmazione}{
  \bl{Astrazione e programmazione}{\iz{
    \item Un linguaggio di programmazione è un linguaggio per descrivere astrazioni
    ...
    \item \`E facile? \`E conveniente? \`E flessibile? Dipende dal linguaggio\dots
  }}
  \bl{Il livello di astrazione col C (e con la progr. strutturata/imperativa)}{\iz{
...
\fr{Alcune caratteristiche/limiti del linguaggio C}{
  \bl{Il C porta ad una visione piuttosto machine-oriented}{\iz{
    ...
    {\iz{
      ...
      \item difficoltà a comprendere/gestire codice scritto da altri
    }}
...
\frs{10}{L'ecosistema dei linguaggi di programmazione}{
  ...
  \bl{Il caso dei linguaggi dinamici\dots}{\iz{
    \item Python, JavaScript: orientati a scripting
    \item molto diffusi (specialmente in applicazioni Web, machine learning, \dots)
  }}
}
...
\fr{I vantaggi della programmazione ad oggetti}{
  ...
  \bl{Le critiche all'OOP}{\iz{
    ...
    \item Altri paradigmi (p.e., funzionale) suggeriscono in parte come farlo\dots
}}
}
...
\fr{Sono questi concetti utili per il problem space?}{
	...
	\item Il CdL è istanza di una classe: la classe dei CdL (con una gestione comune) -- UNIBO ne gestisce centinaia contemporaneamente
    }}
}
...
\fr{OO: problem space, solution space}{
	...
	\item \dots ossia supportano il concetto di ``buon progetto'' che abbiamo discusso
    }}
}
...
\fr{Un oggetto deve nascondere l'implementazione}{
  ...
  \item Tipica struttura di una classe \iz{
   ...
   \item \alert{campi} della classe (i sotto-oggetti di cui è costituito)
   \item implementazione metodi (cosa fa l'oggetto quando riceve messaggi){\iz{
    \item come cambia lo stato (campi)
...
\frs{5}{Qualche nota introduttiva..}{
  \bl{Java}{\iz{
    ...
    \item Al momento il secondo linguaggio più popolare secondo l'indice PYPL{\iz{
	\item fonte: \myurl{https://pypl.github.io/PYPL.html}
    }}
    \item \dots Python no.1, C/C++ e C\# nella top 5
  }}
  \bl{In questo corso}{\iz{
    ...
    \item Java è di fatto equivalente a C\#, che non tratteremo
  }}
}

\fr{Strumenti Java}{
  \bl{Java Development Kit (JDK) -- useremo la versione OpenJDK 17}{\iz{
    \item Insieme di tool per lo sviluppatore
    \item \myurl{https://adoptopenjdk.net/}
  }}
...
\bl{Pricipali versioni}{\iz{
  ...
  \item Java 9-16 (2017-2021): aggiunte minori Java Module System, 
  \item[$\Rightarrow$]  Java 17 (sept. 2021): versione ``long-term support'' (LTS) che useremo
  \item \dots
}}
\bl{Modello di sviluppo attuale}{\iz{
  \item Da Java 9, si ha una nuova release ogni 6 mesi
  \item Java 17 è l'ultima LTS, e si raccomanda di non rimanere indietro
  ...
  \item Esistono anche altre JVM (Eclipse OpenJ9, Amazon Corretto,\dots)
}}
}


%\fr{Motto della lezione}{%  \bx{L'OOP è potente, espressiva, e divertente}
%}
